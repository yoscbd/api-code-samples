...
/* harmony import */ var _wordpress_blocks__WEBPACK_IMPORTED_MODULE_4___default = /*#__PURE__*/__webpack_require__.n(_wordpress_blocks__WEBPACK_IMPORTED_MODULE_4__);
 ...
 //will allow us to pull images data via wp media rest




 //will aloow us to "listen" to a change on a prop and fire a fonction on change
...
function EditComponent(props) {
  (0,_wordpress_element__WEBPACK_IMPORTED_MODULE_0__.useEffect)(function () {
    //Fire this function every time "imgID" prop is changed (see line 34)
    if (props.attributes.imgID) {
      // first check if the user has selected an image
      async function go() {
        const response = await _wordpress_api_fetch__WEBPACK_IMPORTED_MODULE_1___default()({
          //fetch relevant image data with "apiFetch"
          path: `/wp/v2/media/${props.attributes.imgID}`,
          // the root for the media REST + current image id
          method: "GET"
        });
        props.setAttributes({
          imgURL: response.media_details.sizes.pageBanner.source_url
        }); //we update the img url property with the new uploaded image
      }
